    ...
    return (
      <div className="w-full relative flex flex-col gap-4 md:gap-8">
                  ...
                  {items[parseInt(sellerID)]
                    ...
                    .map((item, index) => (
                      <div
                        ...
                        key={`${sellerID}-${item.cart_item_id}-${index}`}
                      >
        ...
        {cartData.length > 0 &&
          cartData
            .map((sellerID) =>
              items[parseInt(sellerID)].filter((item) =>
                item.variant
                  ? item.variant.variant_stock <= 0
                  : item.product.stock <= 0
              )
            ...
            <>
              <Separator />
              <div className="w-full flex flex-col gap-4 md:gap-8 mb-32 md:mb-16">
                <div className="w-full flex flex-col gap-1 lg:gap-2">
                  ...
                  <p className="text-xs lg:text-sm">
                    Produk-produk berikut saat ini tidak tersedia, jika anda
                    ingin memesan produk-produk ini, silahkan lakukan pemesanan
                    pre-order di halaman produk.
                  </p>
                </div>
                ...
                {cartData.map((sellerID) => (
                  <div
                    className="w-full flex flex-col gap-2 px-0 py-0 md:px-4 md:py-2"
                    key={`${sellerID}-unavailable-items`}
                  >
                    {items[parseInt(sellerID)]
                      .filter((item) =>
                        item.variant
                      ...
                      .map((item) => (
                        <div
                          key={`${sellerID}-${item.cart_item_id}-unavailable-items`}
        ...
        <div className="w-[calc(100%-2rem)] md:w-[calc(100%-8rem)] fixed bottom-4 rounded-md border border-input bg-white z-30 px-4 py-2 flex flex-col md:flex-row items-start md:items-center gap-2 lg:gap-4 justify-normal md:justify-between">
