...
export default function VariantChooser({ isOpen, variant }: IVariantChooserProps) {
        ...
        <Modal defaultOpen={isOpen} onClose={variants.handler.closeVariantChooser}>
            <div className="w-full flex flex-col gap-4 max-h-[65vh] overflow-y-auto">
                <div className="flex flex-col gap-2">
                    ...
                    <p className="text-sm">Silahkan pilih salah satu varian produk di bawah ini untuk melanjutkan.</p>
                </div>

                <div className="grid grid-cols-3 gap-2">
                    {variant.variant_item.map((item) => (
                        <Button variant={variants.variantValue ? variants.variantValue.variant_item_id === item.variant_item_id ? "default" : "ghost" : "ghost"} key={item.variant_item_id} onClick={() => variants.handler.onVariantsChange(item)} disabled={item.variant_stock < 1}>
                            {item.variant_name}
                        </Button>
                    ))}
                </div>
                ...
                <div className="w-full rounded-md border border-input flex flex-col gap-2 p-2">
                    <div className="grid grid-cols-2">
                        ...
                        <p>{variants.variantValue ? rupiahConverter(variants.variantValue.variant_price) : rupiahConverter(0)}</p>
                        <p className="font-bold">Stok Varian</p>
                        <p>{variants.variantValue ? variants.variantValue.variant_stock : variant.variant_item.reduce((acc, curr) => acc + curr.variant_stock, 0)}</p>
                    </div>
                </div>

                <Button variant={variants.variantValue ? "default" : "secondary"} onClick={!variants.variantValue ? variants.handler.closeVariantChooser : chooserCallback}>
